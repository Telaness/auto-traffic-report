...
describe("ComparisonRate", () => {
  ...
  describe("高いほど良い指標（既定）", () => {
    it("増加は上向き矢印とグレーで表示する", () => {
      render(<ComparisonRate rate={21.3} />);
      const el = rendered();
    ...
    it("減少は下向き矢印と赤で表示する", () => {
      render(<ComparisonRate rate={-10.1} />);
      const el = rendered();
      expect(el.textContent).toContain("↓");
  ...
  describe("低いほど良い指標（直帰率など）", () => {
    it("減少は改善なので下向き矢印だがグレーで表示する", () => {
      render(<ComparisonRate rate={-10.1} isLowerBetter />);
      const el = rendered();
      expect(el.textContent).toContain("↓");
      expect(el.className).toContain("text-gray-900");
    ...
    it("増加は悪化なので上向き矢印だが赤で表示する", () => {
      render(<ComparisonRate rate={8.4} isLowerBetter />);
      const el = rendered();
      expect(el.textContent).toContain("↑");
      expect(el.className).toContain("text-red-600");
    });
  ...
  it("低いほど良い指標でも増減なしはグレーで表示する", () => {
    render(<ComparisonRate rate={0} isLowerBetter />);
    expect(rendered().className).toContain("text-gray-900");
  });

  it("渡した className を追加する", () => {
    render(<ComparisonRate rate={5} className="text-sm font-medium" />);
    expect(rendered().className).toContain("text-sm font-medium");
  });
});
